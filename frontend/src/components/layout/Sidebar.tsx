...
export const Sidebar: React.FC = () => {
    ...
    <div className="space-y-1 w-full">
      {isExpanded ? (
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="px-3 text-[10px] font-black uppercase tracking-widest text-slate-500/80 mb-2 font-sans flex items-center justify-between whitespace-nowrap"
        >
          <span>{title}</span>
          <span className="w-1.5 h-1.5 rounded-full bg-slate-700/50" />
        </motion.p>
      ) : (
        <div className="w-6 h-px bg-white/[0.08] mx-auto my-2" />
      )}
              ...
              <div className="flex items-center gap-3 relative z-10 min-w-0">
                ...
                {isExpanded && (
                  <motion.span
                    ...
                    className="tracking-tight whitespace-nowrap overflow-hidden text-ellipsis font-medium"
                  >
                    {item.label}
                  </motion.span>
                )}
  ...
  return (
    <div
      ...
      } shrink-0 min-h-[calc(100vh-70px)] relative transition-all duration-300`}
    >
      <motion.aside
        initial={false}
        animate={{
          ...
          boxShadow: isExpanded
            ? '16px 0 35px rgba(0, 0, 0, 0.75)'
            : '2px 0 10px rgba(0, 0, 0, 0.2)',
        }}
        transition={{ type: 'spring', stiffness: 450, damping: 32 }}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        className="absolute top-0 left-0 bottom-0 z-30 flex flex-col glass-panel-luxury border-r border-white/[0.08] p-3 space-y-4 overflow-hidden bg-[#050811]/95 backdrop-blur-3xl min-h-[calc(100vh-70px)] select-none"
      >
        {/* Top Header Row with Pin Toggle */}
        <div className="flex items-center justify-between px-1 pb-2 border-b border-white/[0.06] min-h-[36px]">
          {isExpanded ? (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              <button
                ...
                title={isPinned ? 'Unpin (fold on mouse leave)' : 'Pin sidebar open'}
              >
        ...
        {isExpanded ? (
          ...
          <div
            className="mt-auto w-10 h-10 mx-auto rounded-xl bg-gradient-to-tr from-amber-500/15 to-orange-500/10 border border-amber-500/30 flex items-center justify-center text-amber-400 shadow-md cursor-pointer"
