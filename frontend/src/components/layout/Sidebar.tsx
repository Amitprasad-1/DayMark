...
export const Sidebar: React.FC = () => {
  ...
  const renderNavGroup = (title: string, items: NavItem[]) => (
    <div className="space-y-1">
      <p className="px-3.5 text-[10px] font-black uppercase tracking-widest text-slate-500/80 mb-2 font-sans">
        {title}
      </p>
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full relative flex items-center justify-between px-3.5 py-2.5 rounded-2xl text-xs font-semibold transition-all duration-200 cursor-pointer group outline-none ${
            ...
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 rounded-2xl bg-gradient-to-r from-indigo-600/35 via-indigo-500/20 to-indigo-600/5 border border-indigo-500/40 shadow-[0_4px_20px_rgba(99,102,241,0.18),inset_0_1px_0_rgba(255,255,255,0.15)]"
                />
              )}
              ...
              <div className="flex items-center gap-3 relative z-10">
                <div
                  className={`p-1.5 rounded-xl transition-all ${
                    isActive ? 'bg-indigo-500/25 shadow-sm ring-1 ring-indigo-400/30' : 'group-hover:bg-white/5'
                  }`}
                >
                  <Icon
                ...
                <span
                  className={`relative z-10 px-2 py-0.5 rounded-full text-[10px] font-bold font-mono ${
                    item.id === 'timer' && timerStatus === 'RUNNING'
                      ? 'bg-emerald-400 text-slate-950 animate-pulse font-sans font-black shadow-[0_0_10px_rgba(16,185,129,0.8)]'
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 glass-panel-luxury border-r border-white/[0.08] p-4 space-y-6 shrink-0 min-h-[calc(100vh-70px)] bg-[#050811]/60">
      ...
      <div className="mt-auto p-4 rounded-2xl bg-gradient-to-br from-amber-950/20 via-slate-900/70 to-indigo-950/30 border border-amber-500/20 text-xs space-y-2 relative overflow-hidden shadow-xl group">
        <div className="absolute -top-10 -right-10 w-24 h-24 bg-amber-500/10 rounded-full blur-xl pointer-events-none group-hover:bg-amber-500/20 transition-all duration-500" />
